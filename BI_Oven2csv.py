...
def addBiOven(path):
    txt_file_list = [os.path.join(folder_path, file) for file in os.listdir(folder_path) if os.path.join(folder_path, file).endswith('.csv')]
    for filename in txt_file_list:
        filetemp= pd.read_csv(filename)
        filetemp['BI_Oven']=filename.split('\\')[-1].split('-')[0]
        filetemp.to_csv(filename)
...
def comobine_csv(folder_path):
    csv_file_list = [os.path.join(folder_path, file) for file in os.listdir(folder_path) if os.path.join(folder_path, file).endswith('.csv')]
    reader = csv.DictReader(open(r"C:\Users\yongjiangao\BI\Combine.csv"))
...
if __name__ == '__main__':
    folder_path = r'C:\Users\yongjiangao\BI'
    ...
    addBiOven(folder_path)
    # comobine_csv(folder_path)